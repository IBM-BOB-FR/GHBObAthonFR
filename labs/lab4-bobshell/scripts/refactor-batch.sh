...
echo "$FILES" | while read -r file; do
    ...
    case $REFACTOR_TYPE in
        modernize)
            REFACTOR_CMD="bob \"Refactor $file to use modern language features and best practices while preserving all functionality. Save the refactored code to ${file}.tmp\" --yolo"
            ;;
        optimize)
            REFACTOR_CMD="bob \"Refactor $file to optimize for performance while preserving all functionality. Save the refactored code to ${file}.tmp\" --yolo"
            ;;
        cleanup)
            REFACTOR_CMD="bob \"Refactor $file to remove dead code and fix style issues. Save the refactored code to ${file}.tmp\" --yolo"
            ;;
        security)
            REFACTOR_CMD="bob \"Refactor $file to fix security vulnerabilities while preserving all functionality. Save the refactored code to ${file}.tmp\" --yolo"
            ;;
        *)
    ...
    # Execute refactoring
    # Note: Using --yolo to have Bob save the refactored code directly
    if eval "$REFACTOR_CMD" 2>> "$LOG_FILE" && [ -f "${file}.tmp" ] && mv "${file}.tmp" "$file"; then
        print_success "Done"
