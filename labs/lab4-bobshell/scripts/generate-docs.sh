...
# Note: Using --yolo to have Bob save documentation directly to avoid capturing all output
if bob "Generate comprehensive API documentation for the code in $SOURCE_DIR in $DOC_TYPE format and save to $API_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
if bob "Create architecture documentation for $SOURCE_DIR explaining the system design, components, and data flow in $DOC_TYPE format and save to $ARCH_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
if bob "Create comprehensive usage examples for all public functions and classes in $SOURCE_DIR with code snippets and explanations and save to $EXAMPLES_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
- License information
Format as professional GitHub README with badges and sections and save to $README_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
- Commit messages organized by category and save to $CHANGELOG_DOC" --yolo 2>> "$LOG_FILE"; then
        print_success "Done"
...
- Community guidelines and save to $CONTRIBUTING_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
if bob "Create a CODE_OF_CONDUCT.md following the Contributor Covenant standard and save to $CODE_OF_CONDUCT_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
- Performance issues
- Debugging tips and save to $TROUBLESHOOTING_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
...
- Common problems
- Best practices and save to $FAQ_DOC" --yolo 2>> "$LOG_FILE"; then
    print_success "Done"
